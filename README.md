...
Настраивается через меню (`c`) или вручную. Запускается каждый день в 04:00, обновляет только если доступна новая версия.
...
Ручной запуск в автоматическом режиме:
```sh
